...
import type { LabCommand, LabEvent } from "./protocol";
...
export type ScenarioState =
  ...
  | {
      ...
      readonly revealed: boolean;
      /**
       * Sticky progress flags for the four solubility objective steps; each latches true once
       * reached and never reverts. Absent right after LOAD_SCENARIO's initial construction only
       * if a caller builds the state by hand; loadScenario always sets all four to false.
       */
      readonly milestones?: SolubilityMilestones;
    };

/** Sticky solubility-scenario progress: see the `solubility` ScenarioState variant. */
export interface SolubilityMilestones {
  readonly addedEnoughSolute: boolean;
...
// Lives in publicTypes.ts; re-exported here so every existing `./types` import keeps working.
export type { ContentsView, PublicContainer, PublicLabState, PublicScenario, PublicSolidDeposit } from "./publicTypes";
