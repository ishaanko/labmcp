  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <ul className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto">
        {rows.map((row) => (
          <li key={row.seq} className="flex items-start gap-2 rounded-md px-1.5 py-1 text-sm hover:bg-muted/40">
            <SourceBadge actor={row.actor} className="mt-0.5" />
            <p className="min-w-0 flex-1 text-foreground">{row.text}</p>
            <span className="tabular shrink-0 text-xs text-muted-foreground">{row.clockS.toFixed(0)}s</span>
          </li>
        ))}
      </ul>
    </div>
