...
/** pH probe: a meter box with the reading, plus a rod dipping down to the electrode tip. */
export function PHMeter(props: InstrumentProps) {
  const rodOpacity = props.attached ? 1 : 0.45;

  return (
    <VesselFrame viewBoxWidth={VIEW_W} viewBoxHeight={VIEW_H} size={props.size ?? 120} label="pH meter" hovered={false}>
      <rect x={28} y={10} width={64} height={36} rx={8} fill={GLASS_FILL} stroke={OUTLINE} strokeWidth={2.5} />
      <text x={60} y={33} fontSize={13} textAnchor="middle" fill="#e6e6ee" className="tabular-nums">
        {props.reading ?? "--"}
      </text>
